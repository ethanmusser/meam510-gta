#line 1 "c:\\Users\\ethan\\Google Drive\\School\\11. Masters III - Fall 2021\\Mechatronics (MEAM 510)\\Project\\code\\meam510-gta\\esp32-slave\\vive510.cpp"
/**
 * source for vive510 class.
...
void IRAM_ATTR Vive510::pulseISR(uint32_t us)
{
    ...
    if (m_vivestatus == VIVE_LOCKEDON)
        processPulse();
}

uint32_t mymod(uint32_t a, uint32_t bnum, uint32_t bdenom)
{ //could use fmod here...
    while (a > (uint32_t)0xffffffff / bdenom)
        a -= bnum; // wrap case, know that bnum is divisible
    return a - (bnum * (a * bdenom / bnum) + bdenom / 2) / bdenom;
}

uint32_t Vive510::alignment(uint32_t timeus, uint32_t freq)
{ //  positive modulo  closest to either end
...
{
    if (alignment(m_usRising - m_currentRising, 60) < 300)
        return true;
    else
        return false;
}
...
{
    if (alignment(m_usRising - m_currentRising - 1000000 / 120, 60) < 300)
        return true;
    else
        return false;
}
...
void Vive510::processPulse()
{
    ...
    if (m_lastFalling != m_usFalling)
    {
        ...
        if (pulsewidth > m_sweepWidth)
        {
            if (isKPulse())
            {
                if (m_usFalling - m_lastFalling > 8500)
                { // missed a 120Hz sync pulse
                ...
                if (checkflag > 0)
                {
                    ...
                    if (m_currentKPulse - m_currentJPulse < 0)
                        swapXY();
                }
                if (--checkflag < 0)
                    checkflag = 0;
...
uint32_t Vive510::sync(int reps)
{
    ...
    while (millis() - startms < 1000 / 60)
    { // look at 1st 2 JorK pulses
        if (m_lastFalling != m_usFalling)
        {
            m_lastFalling = m_usFalling;
    ...
    while (millis() - startms < reps * 1000 / 60)
    {
        if (m_lastFalling != m_usFalling)
        {
            ...
            {
                i++;
                if (alignment(m_usRising - currentKRising, 60) < 200)
                { // aligns with currentK
                    updateKPulse(pulsewidth);
                }
                else if (alignment(m_usRising - currentKRising - 1000000 / 120, 60) < 200)
                { // aligns with currentK
                    updateJPulse(pulsewidth);
                }
                else
